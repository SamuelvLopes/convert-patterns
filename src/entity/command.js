...
class Command {
  ...
  execute() {
    throw new Error(
      "O método execute deve ser implementado na subclasse Command."
    );
  }
}
class FileIterator {
...
class FileConverterStrategy {
  execute(receiver) {
    throw new Error(
      "O método execute deve ser implementado na subclasse FileConverterStrategy."
    );
  }
}
      ...
      .then(() => {
        receiver.client
          .sendFile(
            ...
            receiver.file + ".png",
            "file_name",
            "Segue arquivo convertido: http://localhost:1603/" +
              receiver.file +
              ".png"
          )
          .then((result) => {
...
class docxConvertCommand extends FileConverterStrategy {
  ...
  transformarNomeDoArquivo(caminhoOriginal, novaExtensao) {
    ...
    if (caminhoOriginal.endsWith(".document")) {
      ...
      const nomeDoArquivoSemExtensao = caminhoOriginal
        .split("/")
        .pop()
        .slice(0, -9);
